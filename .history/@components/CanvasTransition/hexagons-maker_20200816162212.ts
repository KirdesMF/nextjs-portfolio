...
const updateCanvasHexagons = (hexmap: THexagons[]) => {
   hexmap.forEach((hex) => {
      ...
      else if (hex.scaleIteration < hex.scaleTotalIteration) {
         hex.scaleIteration++;
      ...
      } else if (hex.colorIteration < hex.colorTotalIteration) {
         hex.colorIteration++;
         hex.color.l = Utils.ease.easeInCirc({
            i: hex.colorIteration,
            s: 1,
            c: Math.min(hex.color.l, hex.nextColor.l),
            t: hex.colorTotalIteration,
         });
      }

      if (hex.colorIteration === hex.colorTotalIteration) {
         console.log(hex.color.l);
      }
   });
};
